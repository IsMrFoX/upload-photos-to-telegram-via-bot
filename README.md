...
#### 5. send_all_imgs --> выкладывает в телеграм канал все картинки в бесконечном цикле с задаваемым числом и временным промежутком

Переменные окружения *'tg_channel_id'* - для телеграм канала и *'telegram_bot_token'* - для бот токена. 1. Аргумент - сколько картинок выкладывать, по умолчанию 1 картинка. 2. Аргумент - Через сколько минут выложить следующие. 3. Аргумент измерения времени через которое выложить картинку: "s", "m", "h".
